...
const botname = process.env.BOTNAME || 'Prime AI';
...
if (antilink === 'TRUE' && antilinkall === 'TRUE' && body.includes('http') && !Owner && isBotAdmin && !isAdmin && m.isGroup) { 
 ...
 client.sendMessage(m.chat, {text:`Auf wiedersehen:\n\n@${ki.split("@")[0]}, sending links is prohibited in this group!`, contextInfo:{mentionedJid:[ki]}}, {quoted:m}); 
    ...
    if (cmd && !m.isGroup) {
      console.log(chalk.black(chalk.bgWhite("[ PRIME-AI ]")), color(argsLog, "turquoise"), chalk.magenta("From"), chalk.green(pushname), chalk.yellow(`[ ${m.sender.replace("@s.whatsapp.net", "")} ]`));
    ...
    if (cmd) {
      ...
      case "help":
        case "menu":
let cap = `╭════〘  𓊈𒆜 *PRIME-* *AI* 𒆜𓊉═⊷❍
┃╭─────────
┃✯│𝙐𝙨𝙚𝙧 :   ${m.pushName}
┃✯│𝙋𝙞𝙣𝙜 :   ${dreadedspeed.toFixed(4)} 𝐦𝐬`});
┃✯│𝘿𝙖𝙩𝙖𝙗𝙖𝙨𝙚 : None
┃╰─────
╰─────────❍
         𝘿𝙊𝙒𝙉𝙇𝙊𝘼𝘿
...
client.sendMessage(m.chat, { image: { url: 'https://telegra.ph/file/54ba01cf94a6b8bdc3f50.jpg' }, caption: cap, fileLength: "9999999999"}, { quoted: m })
} else if (menu === 'LINK') {
client.sendMessage(m.chat, {
                        text: cap,
                        contextInfo: {
                            externalAdReply: {
                                ...
                                thumbnail: fs.readFileSync('./prime-ai.jpg'),
                                sourceUrl: 'https://wa.me/254740782927?text=ʜᴇʟʟᴏ+𝐌𝐈𝐊𝐄+Nihostie+Bot+Mkuu+🥲',
                                mediaType: 1,
...
case "ping": case "speed": { 
 ...
 case "runtime": { 
                 m.reply (`PRIME AI has been running for ${runtime(process.uptime())}`) 
 } 
 break;

case "alive": { 
  
 client.sendMessage(m.chat, { video: { url: 'https://telegra.ph/file/1fec76a969c9c47e6e536.mp4' }, caption: `Hello ${m.pushName}, 𝐏𝐑𝐈𝐌𝐄 𝐀𝐈 is alive since  ${runtime(process.uptime())}`, fileLength: "9999999999898989899999999" }, { quoted: m }); 
 }
break;
case 'apk': {
...
client.sendMessage(from, buttonMessage, { quoted: m }) 
 ...
 break 
   ...
   client.sendMessage(m.chat, { image: buffer, caption: `Converted by Prime ai! 🦄`}, { quoted: m }) 
...
case "movie": 
 ...
 break;
          ...
          case "prime": case "owner": case "mike": 
 client.sendMessage(m.chat, { image: { url: 'https://telegra.ph/file/9f7460efc663367b5154c.jpg' }, caption: `Say no more 😌 here is 𝐌𝐢𝐤𝐞 contact +254740782927 🥱` }, {quoted: m}); 
...
case "getvar": 
 if (!Owner) throw NotOwner;  
     ...
     let stoy = '*Below Are Heroku Variables For Prime:*\n\n';  
 ...
 break;
        ...
        case "gpt": case "gpt": 
            ...
            if (!text) return reply("Hello am 𝐏𝐑𝐈𝐌𝐄 an Ai developed by 𝐌𝐢𝐤𝐞, how can I help you today?");
...
async function handleGPTMessage(text, m) {
  if (!text) return m.reply("Hello am 𝐏𝐑𝐈𝐌𝐄 an Ai developed by 𝐌𝐢𝐤𝐞, how can I help you today?");
